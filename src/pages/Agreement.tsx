...
const Agreement = () => {
  ...
  return (
    <main className="container mx-auto px-4 py-12">
      ...
      <section
        id="agreement"
        className="p-6 border rounded-xl bg-white text-black"
      >
        ...
        <p className="text-sm mt-2">
          Between Digital Lift Hub ("Agency") and the Client.
        </p>
        <h3 className="mt-4 font-semibold">Scope of Services</h3>
        <p className="text-sm">
          Social Media Marketing (6+ posts/month with captions), Reels Editing
          (4/month), Branding & Design (logo/templates/posters/stories),
          Strategy & Campaign Planning, Analytics & Reporting.
        </p>
        <h3 className="mt-4 font-semibold">Deliverables</h3>
        <p className="text-sm">
          Weekly rollouts, end-of-month performance report. Weekly Off: Sunday.
        </p>
        <h3 className="mt-4 font-semibold">Revisions</h3>
        <p className="text-sm">
          Up to 2 revisions per asset. Additional revisions billed separately.
        </p>
        ...
        <p className="text-sm">
          Total Monthly Cost: ₹25,000. Advance ₹12,500 on onboarding; Balance
          ₹12,500 by month-end. Payment via UPI/NEFT/Bank Transfer.
        </p>
        <h3 className="mt-4 font-semibold">Termination</h3>
        <p className="text-sm">
          Either party may terminate with 7-day written notice. Dues for work
          completed remain payable.
        </p>
      </section>
      ...
      <section className="mt-8 p-6 border rounded-xl">
        ...
        <form
          ...
          aria-label="Agreement acceptance form"
        >
          <input
            type="text"
            name="_gotcha"
          ...
          <label className="md:col-span-3 flex items-center gap-2 text-sm">
            <input type="checkbox" required /> I accept the terms in the Client
            Agreement.
          </label>

          {/* Submit button */}
          <div className="md:col-span-3 relative flex justify-center">
  <button className="px-4 py-2 rounded-md bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]">
    Submit Acceptance
  </button>

            {/* Popup inside button area */}
            {status === "SUCCESS" && (
              <div className="absolute -top-10 left-0 bg-green-500 text-white px-3 py-1 rounded shadow text-sm">
                ✅ Sent Successfully
              </div>
            )}
            {status === "ERROR" && (
              <div className="absolute -top-10 left-0 bg-red-500 text-white px-3 py-1 rounded shadow text-sm">
                ❌ Failed to Send
              </div>
            )}
          </div>
